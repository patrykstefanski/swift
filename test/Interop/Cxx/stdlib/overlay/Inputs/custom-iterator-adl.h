...
// operators (==, -) found via ADL, and member operator+=. Overload resolution
// passes accurate operand types (RecordType&, DifferenceType), so the member
// operator+= is viable; this case pins the member-operator variation.
...
// MARK: Case 5 - Random access iterator inheriting its operators (==, -, +=)
// from a base class. The operators found by overload resolution take the base
// type, which fails validation for the derived type, so forwarding wrappers
// are synthesized. The operator+= wrapper must take its left operand by
// reference (it mutates the iterator) and return void.
...
struct IteratorBase {
  ...
  IteratorBase(int value) : value(value) {}

  friend bool operator==(const IteratorBase &lhs, const IteratorBase &rhs) {
    return lhs.value == rhs.value;
  }
  friend int operator-(const IteratorBase &lhs, const IteratorBase &rhs) {
    return lhs.value - rhs.value;
  }
  friend IteratorBase &operator+=(IteratorBase &lhs, int v) {
    lhs.value += v;
    return lhs;
  }
};

struct InheritedOpsRACIterator : IteratorBase {
  using iterator_category = std::random_access_iterator_tag;
  using value_type = int;
  using pointer = int *;
  using reference = const int &;
  ...
  InheritedOpsRACIterator(int value) : IteratorBase(value) {}
  InheritedOpsRACIterator(const InheritedOpsRACIterator &) = default;

  const int &operator*() const { return value; }

  InheritedOpsRACIterator &operator++() {
    value++;
    return *this;
  }
  InheritedOpsRACIterator operator++(int) {
    auto tmp = *this;
    value++;
    return tmp;
  }
